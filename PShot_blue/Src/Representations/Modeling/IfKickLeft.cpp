...
#include "IfKickLeft.h"
/*返回0 说明球门柱不在视野的左边，
返回1视野的左边有一个门柱，且障碍物在左前方，
返回2视野的左边有一个门柱，且障碍物在正前方，
返回3 视野的左边有门柱，且障碍物在视野的右前方

*/
int  IfKickLeft(const ObstacleModel& theObstacle,  const GoalPercept& theGoalPercept )
{
    ...
    else if(theGoalPercept.goalPosts.size() ==1)
    {
       ...
       if(theGoalPercept.goalPosts[1].positionInImage.x() > 160.f &&  theGoalPercept.goalPosts[1].positionInImage.x() > 160.f )
        return 0;
